...
final class HashChainVerificationTest extends TestCase
{
    ...
    public function testUnchangedHistoricalHashFormatAndValidChain(): void
    {
        ...
        foreach ($events as $event) {
            $historicInput = implode('|', [(string) $event->getId(), '{"message":"échantillon"}', '1770000000', '', '[]', 'test-ip', $event->getHashPrevious()]);
            self::assertSame(hash_hmac('sha256', $historicInput, 'test-only-secret'), $event->getHashSelf());
        }
    ...
    private function chain(): array
    {
        ...
        $chain = new HashChainService($registry, 'test-only-secret');
        $events = [];
        $previous = str_repeat('0', 64);
        for ($i = 0; $i < 2; ++$i) {
            $event = (new PlantEvent())->setPayload(['message' => 'échantillon'])->setOccurredAt(new \DateTimeImmutable('@1770000000'))->setHashPrevious($previous)->setIpAddress('test-ip');
