...
validate_required_env() {
  ...
  missing=""
  for var in APP_SECRET DATABASE_URL JWT_PASSPHRASE MERCURE_JWT_SECRET STRIPE_SECRET_KEY STRIPE_WEBHOOK_SECRET STRIPE_PRICE_GROWTH STRIPE_PRICE_PRO STRIPE_PRICE_SCALE STRIPE_PRICE_ENTERPRISE; do
    eval "val=\${${var}:-}"
    if [ -z "$val" ]; then
      missing="${missing} ${var}"
    fi
